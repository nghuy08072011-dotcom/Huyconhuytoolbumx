...
try:
    ...
    # --- THÊM CÁC THƯ VIỆN CẦN THIẾT CHO HÀM MỚI ---
...
# --- KHAI BÁO BIẾN TOÀN CỤC CHO HÀM QUẢN LÝ VỐN ---
base_bet = 1.0
multiplier = 2.0
run_mode = "real"
...
# --- CÁC HÀM HỖ TRỢ CHO PROMPT SETTINGS ---
def slow_print(text, style=""):
...
def fetch_balances_3games(retries=2, timeout=5):
    """
    Hàm giả lập lấy số dư để tương thích với prompt_settings.
    Trả về None để kích hoạt nhập thủ công.
    """
...
# ==============================================================================
#  HÀM QUẢN LÝ VỐN (YÊU CẦU THÊM MỚI - GIỮ NGUYÊN 100%)
# ==============================================================================
def prompt_settings() -> bool:
    ...
    use_tay_system = Prompt.ask("   Bạn có muốn [bold]chia vốn theo 'tay'[/bold] không? (y/n)", choices=["y", "n"], default="n")
    reinvest_profit = False
    _reinvest_num_tay = None
    _reinvest_multiplier = None
    if use_tay_system.lower() == 'y':
        ...
        if current_capital is None:
            ...
            current_capital = FloatPrompt.ask("   Nhập số vốn BUILD của bạn", default=100.0)
        console.print(f"   [green]Vốn hiện tại của bạn là: {current_capital:,.2f} BUILD[/green]")
        num_tay = IntPrompt.ask("   [1] 🛡️ Bạn muốn chia vốn thành bao nhiêu 'tay' (chũi thua cao nhất cộng 1)?", default=4)
        multiplier = FloatPrompt.ask("   [2] 📈 Nhập hệ số nhân khi thua (gấp thếp)", default=12.0)
        if multiplier <= 1:
        ...
        try:
            ...
            for i in range(num_tay):
                ...
                tay_table.add_row(f"{i+1}", f"{current_tay_bet:,.4f}", f"{total_bet_so_far:,.4f}")
            console.print(tay_table)
        except (OverflowError, ValueError) as e:
            console.print(f"[red]Lỗi tính toán cược gốc: {e}. Sử dụng cược gốc mặc định là 1.0[/red]")
            base_bet = 1.0
        reinvest_choice = Prompt.ask("\n   Bạn có muốn tự động [bold]tăng cược khi có lãi[/bold] (tái đầu tư lợi nhuận) không? (y/n)", choices=["y", "n"], default="n")
...
def load_config():
    # Tích hợp prompt_settings trực tiếp vào luồng
    # Nếu file config tồn tại, đọc URL trước
    url_game = ""
    bet_type = "BUILD"
    ...
    if os.path.exists(CONFIG_FILE):
        try:
            with open(CONFIG_FILE, "r") as f: 
                old_cfg = json.load(f)
                url_game = old_cfg.get("url_game", "")
                bet_type = old_cfg.get("bet_type", "BUILD")
        except: pass
    
    if not url_game:
        console.print(Panel("[bold yellow]THIẾT LẬP KẾT NỐI[/]", border_style="cyan"))
    ...
    console.print(Panel("[bold yellow]CẤU HÌNH BỔ SUNG[/]", border_style="cyan"))
    max_lose_streak = int(console.input("[bold cyan]Max Thua (để cắt lỗ): [/]").strip() or 10)
    stop_profit = float(console.input("[bold cyan]Mục tiêu Lãi (Enter = 1000): [/]").strip() or 1000)
    stop_loss = float(console.input("[bold cyan]Giới hạn Lỗ (Enter = 500): [/]").strip() or 500)
    play_rounds = int(console.input("[bold cyan]Số ván chơi (0 = vô hạn): [/]").strip() or 0)
    pause_rounds = int(console.input("[bold cyan]Số ván nghỉ khi đạt mốc: [/]").strip() or 0)
    ...
    config = {
        "url_game": url_game,
        "bet_type": bet_type,
        "base_bet": base_bet,   # Lấy từ global sau khi chạy prompt_settings
        "multiplier": multiplier, # Lấy từ global
...
def main():
    ...
    # Sử dụng base_bet và multiplier từ hàm prompt_settings (đã lưu vào cfg)
